...
validate_migration() {
    ...
    # --- GitHub branches ---
    local gh_branches
    gh_branches=$(gh api "/repos/$github_org/$github_repo/branches" --paginate 2>/dev/null) || gh_branches="[]" 
    ...
    while IFS= read -r branch; do
        [[ -n "$branch" ]] && gh_branch_array+=("$branch")
    done < <(echo "$gh_branches" | jq -r '.[].name // empty')
    ...
    while IFS= read -r branch; do
        [[ -n "$branch" ]] && ado_branch_array+=("$branch")
    done < <(echo "$ado_branch_response" | jq -r '.value[].name' | sed 's|^refs/heads/||')
